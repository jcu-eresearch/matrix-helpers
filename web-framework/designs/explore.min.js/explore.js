...
(function($) {
  ...
  var reduceMotion = window.matchMedia("(prefers-reduced-motion)").matches
  ...
  // Stop animations on small screens, or if disabled, or if user
  // is asking to reduce motion
  if ($(window).width() < 768 || !$('body.animations-on') || reduceMotion) {
    disableAnimations()
  }
  ...
  // find every lightboxable image
  var $boxables = $('.link-gallery')

  // boxables inside a "parent" should share a lightbox gallery
  $('.link-gallery-parent').each( function(index, parent) {
    var $children = $(parent).find('.link-gallery') // this parent's boxables
    $children.magnificPopup(magnificConfig) // make a lightbox for them
    $boxables = $boxables.not($children) // remove them from the Big List
  })

  // anything left without a parent goes into "dregs" lightbox gallery
  $boxables.magnificPopup(magnificConfig)

  // Link rewriting ///////////////////////////////////////
  // Retarget external links to specific types of browser-viewable files
  $('a[href$=".pdf"],a[href$=".PDF"]').not('.btn,.no-change').each(function() {
    // Only rewrite if we don't have a target already
    if (!this.target) {
      this.target = '_blank'
      $(this).append('<small class="icon-opennewwindow m-l-1/4 hidden-print" title="Opens in new window"></small>')
    }
  })
